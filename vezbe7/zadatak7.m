...
for t=1:Nt
    ...
    if (V(t+1)>=Vspike) %provera da li je spike dostigao maksimum
        V(t+1) = c; %reset posle generisanja spike
        u(t+1) = u(t+1)+d;
    end
end
%iscrtavanje rezultata simulacije
subplot(3,1,1);
plot(T,I);
...
subplot(3,1,2);
plot(T,V);
title('Akcioni potencijal - spikes');
xlabel('Vreme [ms]');
